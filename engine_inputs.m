function [inputs, properties_fuel, properties_oxidizer] = engine_inputs()
    ...
    inputs.delta_p_cooling_channels = 5e5;  % [Pa]      % (Research and ask Thrust Chamber)
    ...
    inputs.delta_p_pump_LCH4 = 12e6;        % [Pa]      % (First iteration, needs only be decent)
    inputs.delta_p_pump_LOx = 10e6;         % [Pa]      % (First iteration, needs only be decent)

    %% Cooling assumptions
    inputs.Q_dot = 1e6;                     % [J/s]     % (Ask Thrust Chamber)

    %% Fuel properties (input here initial properties
    properties_fuel.T = 110;                % [K]       % (First iteration, needs only be decent)
    properties_fuel.p = 2e5;                % [Pa]      % (First iteration, needs only be decent)
    ...
    %% Fuel properties (input here initial properties);
    properties_oxidizer.T = 90;             % [K]       % (First iteration, needs only be decent)
    properties_oxidizer.p = 2e5;            % [Pa]      % (First iteration, needs only be decent)

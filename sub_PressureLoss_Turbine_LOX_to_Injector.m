function [properties, key_values] = sub_PressureLoss_Turbine_LOX_to_Injector(inputs, properties)

    D   = inputs.D_TurbineOx_to_Injector;
    A   = (D/2)^2 * pi;
    L   = inputs.L_TurbineOx_to_Injector;
    K   = 0.6;          % Combined minor-loss coefficient for a short fuel feedline including fittings and control-system losses only
    ...
    %% Main Fuel Valve (ball valve) — located at line midpoint
    % K = 3*f_T, Crane clean-commercial-steel f_T at D = 50 mm -> f_T = 0.019
    K_valve = 0.057;
    N_half  = N / 2;

    %% Check Valve (swing check, downstream of Main Fuel Valve, before injector)
    % K = 100*f_T, Crane clean-commercial-steel f_T at D = 50 mm -> f_T = 0.019
    K_check = 1.90;
    ...
    for i = 1:N
        ...
        % Main Fuel Valve, applied once at the line midpoint
        if i == N_half
        ...
        % Check Valve, applied downstream of the Main Fuel Valve
        if i == N_check
            rho_c        = py.CoolProp.CoolProp.PropsSI('D', 'P', properties.p, 'T', properties.T, 'Methane');
            v_c          = inputs.m_dot_fuel / (rho_c * A);
            q_c          = 0.5 * rho_c * v_c^2;

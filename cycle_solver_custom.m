function [key_values, properties_flow] = cycle_solver_custom(inputs, pf0, po0, do_plot)
    ...
    % Allow optional programmatic inputs/properties so external sweep routines
    % can call the solver with customized `inputs`.
    if nargin < 1 || isempty(inputs)
        [inputs, pf0, po0] = engine_inputs();
    else
        % Ensure property structs exist
        if nargin < 2 || isempty(pf0) || isempty(po0)
            [~, pf0, po0] = engine_inputs();
        end
        % Recompute derived values in `inputs` that depend on p_CC_req, ROF, etc.
...
function [key_values, properties_flow, W_LCH4, W_LOx, P_LCH4, P_LOx] = run_cycle(inputs, properties_fuel, properties_oxidizer)
    ...
    % Expose pump delta pressures so external callers can visualize them
    key_values.delta_p_pump_LCH4 = inputs.delta_p_pump_LCH4;
    key_values.delta_p_pump_LOx = inputs.delta_p_pump_LOx;
end

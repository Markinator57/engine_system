function [properties, key_values] = sub_Turbine_LCH4(inputs, properties)
    ...
    p_in  = properties.p;
    p_out = inputs.p_turbine_LCH4_out;
    ...
    h_out       = h_in - inputs.eta_turbine_LCH4 * (h_in - h_out_s);
    W_available = inputs.m_dot_fuel * (h_in - h_out);

    if W_available < inputs.P_turbine_LCH4_needed
        warning('LCH4 turbine: available %.1f W < needed %.1f W — pump pressure too low to close cycle', ...
                W_available, inputs.P_turbine_LCH4_needed);
    else
        warning('LCH4 turbine: surplus: available %.1f W > needed %.1f W (excess %.1f W)', ...
                W_available, inputs.P_turbine_LCH4_needed, W_available - inputs.P_turbine_LCH4_needed);
    end
